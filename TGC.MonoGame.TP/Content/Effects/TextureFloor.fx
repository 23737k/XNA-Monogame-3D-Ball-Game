...
float 4x4 World;
float 4x4 WorldViewProjection;
float Tiling;

texture Texture;
sampler 2D textureSampler = sampler_state
{
    Texture = (Texture);
    MagFilter = Linear;
    MinFilter = Linear;
...
VertexShaderOutput BaseVS(in VertexShaderInput input)
{
...
float4 BasePS(VertexShaderOutput input) : COLOR
{
...
WorldVertexShaderOutput WorldVS(in WorldVertexShaderInput input)
{
...
float4 WorldPS(WorldVertexShaderOutput input) : COLOR
{
...
technique BaseTechnique 
{
    pass P0 
    {
        VertexShader = compile VS_SHADERMODEL BaseVS();
        PixelShader = compile PS_SHADERMODEL BasePS();
    }

}

technique WorldTechnique 
{
    pass P0
    {
        VertexShader = compile VS_SHADERMODEL WorldVS();
        PixelShader = compile PS_SHADERMODEL WorldPS();
    }
}